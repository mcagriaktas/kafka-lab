...
# ----------------------------
echo ""
echo "====[ Creating Keycloak certificate ]===="
KEYCLOAK_SANS="DNS:keycloak,DNS:keycloak.$DNS_NAME,DNS:localhost,IP:127.0.0.1"

# Generate Keycloak private key (DO NOT touch rootCA.key here!)
openssl genrsa -out "$KEYS_DIR/keycloak/keycloak.key" "$KEY_SIZE"

# Generate CSR
openssl req -new \
    -key "$KEYS_DIR/keycloak/keycloak.key" \
    -out "$KEYS_DIR/keycloak/keycloak.csr" \
    -subj "/CN=keycloak.$DNS_NAME/OU=Kafka/O=Example/L=Bursa/ST=TR/C=TR"

# Sign with Root CA
openssl x509 -req \
    -CA "$ROOTS_DIR/rootCA.crt" \
    -CAkey "$ROOTS_DIR/rootCA.key" \
    -in "$KEYS_DIR/keycloak/keycloak.csr" \
    -out "$KEYS_DIR/keycloak/keycloak.crt" \
    -days "$VALIDITY_DAYS" \
    -CAcreateserial \
    -extensions v3_req \
    -extfile <(echo -e "[v3_req]\nsubjectAltName=$KEYCLOAK_SANS")

rm -f "$KEYS_DIR/keycloak/keycloak.csr"
